...
//data->cmd = data->prev_cmd + a0*data->error - a1*data->prev_error
...
{
	for (uint16_t j=0; j<1000 ; j++)    //init speed consign
	{
		speedRef_L[j]=0;  speedRef_R[j]=0;
	}

  uint16_t t100Hz_angle = (uint16_t) 635*((float) abs(angle))/((float) speed_ref); //set time at constant speed

  if(t100Hz_angle>1000) return 1; //error flag if overflow
	...
	if(angle > 0)
	{
		...
		{
			speedRef_L[j]=-6000; speedRef_R[j]=-6000;
		}
		for (uint16_t j=2; j<t100Hz_angle-1 ; j++)
		{
			speedRef_L[j]=-speed_ref; speedRef_R[j]=-speed_ref;
		}
		for (uint16_t j=t100Hz_angle-1; j<t100Hz_angle; j++)
		{
			speedRef_L[j]=0; speedRef_R[j]=0;
		}
	}
	...
	if(distance!=0)
	{
		uint16_t t100Hz_dist = (uint16_t) 260*((float) abs(distance))/((float) speed_ref); //error flag if overflow

    if(t100Hz_dist>1000)  return 1; //error flag if overflow
		...
		if(distance >500)
		{
			...
			for (uint16_t j=2; j<t100Hz_dist-1; j++)
			{
				speedRef_L[j]=-speed_ref; speedRef_R[j]=speed_ref;
			}
		...
		else if(distance>250) // 250 < d <500mm
		{
			for (uint16_t j=0; j<2; j++)
			{
				speedRef_L[j]=-6000; speedRef_R[j]=6000;
		...
		else if(distance>0) // 0 < d <250mm
		{
			for (uint16_t j=0; j<t100Hz_dist-1; j++)
			{
				speedRef_L[j]=-6000; speedRef_R[j]=6000;
			}
			for (uint16_t j=t100Hz_dist-1; j<t100Hz_dist; j++)
		...
		else
		{
			...
			else if(distance>=-250 && distance <0) // 0 < d <250mm
			{
				for (uint16_t j=0; j<t100Hz_dist-1; j++)
				{
					speedRef_L[j]=6000; speedRef_R[j]=-6000;
